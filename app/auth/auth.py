...
from app.auth.schemas import SUsername, SAuthRefresh
...
async def delete_refresh_token(data: SAuthRefresh, request) -> None:
    token_data = get_refresh_token_data(data)
    await AuthDAO(token_data, request).delete()


async def validate_refresh_token(data: SAuthRefresh, request) -> str:
    token_data = get_refresh_token_data(data)
